...
class DE():
    ...
    def selection(self, fit_value, fit_cons1, fit_cons2):
        ...
        for i in range(self.pop_size*2):
            newfit[i] = np.square(min(0, fit_cons1[i])) + np.square(min(0, fit_cons2[i]))

        newpop = np.zeros((self.pop_size, self.chrom_length))
        for i in range(self.pop_size):
                if self.judge(fit_cons1[i], fit_cons2[i]):
                    if self.judge(fit_cons1[i + self.pop_size], fit_cons2[i + self.pop_size]):
                        if fit_value[i] < fit_value[i + self.pop_size]:  # 最小化<, 最大化>
                            # if fit_value[i] > fit_value[i + self.pop_size]:  # 最小化<, 最大化>
                            newpop[i] = self.pop[i]
                        else: newpop[i] = self.pop[i + self.pop_size]
                    else: newpop[i] = self.pop[i]
                elif self.judge(fit_cons1[i + self.pop_size], fit_cons2[i + self.pop_size]):
                    newpop[i] = self.pop[i + self.pop_size]
                else:
                    if random.random() < stochastic_ranking_pf:
                        if fit_value[i] < fit_value[i + self.pop_size]:  # 最小化<, 最大化>
                            newpop[i] = self.pop[i]
                        else: newpop[i] = self.pop[i + self.pop_size]
                    else:
        ...
        self.fitfirst.append(SurrogateObj.predict(popa)[rank[0]])  # 最小化0, 最大化-1
