...
class MOE():
    ...
    def condition1(self):
        theta1 = 1e-10
        fit_value = SurrogateObj.predict(self.pop)
        fit_cons1 = SurrogateCons1.predict(self.pop)
        fit_cons2 = SurrogateCons2.predict(self.pop)
        arr = []
    ...
    def model_1(self):
        ...
        cfit1 = SurrogateObj.predict(C)
        cfit2 = SurrogateCons1.predict(C)
        cfit3 = SurrogateCons2.predict(C)
        R = self.ndsort(C, cfit1, cfit2, cfit3)
        qfit = SurrogateObj.predict(Q)
        for i in range(len(R)):
            ...
            if sum(arr) == 0:
                ...
                Q[weizhi] = R[i]
                qfit[weizhi] = SurrogateObj.predict([R[i]])
            elif SurrogateCons1.predict([R[i]])>=0 and SurrogateCons2.predict([R[i]])>=0:
                index = np.where(arr == 1)
                afit = qfit[index]
                weizhi = np.where(afit == max(afit))
                Q[index[0][weizhi]] = R[i]
                qfit[index[0][weizhi]] = SurrogateObj.predict([R[i]])
            else:
                index = np.where(arr == 1)
    ...
    def model_2(self):
        ...
        for i in range(1):
            if sum(arr) == 0:
                ...
                weizhi = np.where(arr == 1)
                Q[weizhi] = R[i]
            elif SurrogateCons1.predict([x_1]) >= 0 and SurrogateCons2.predict([x_1]) >= 0:
                index = np.where(arr==1)
                afit = qfit[index]
                weizhi = np.where(afit == max(afit))
                Q[index[0][weizhi]] = x_1
                qfit[index[0][weizhi]] = SurrogateObj.predict([x_1])
            else:
                index = np.where(arr==1)
                weizhi = np.random.randint(0,len(index[0]))
                Q[index[0][weizhi]] = x_1
    ...
    def model_3(self):
        ...
        cfit3 = SurrogateCons2.predict(C)
        R = self.ndsort(C, cfit1, cfit2, cfit3)
        rfit1 = SurrogateObj.predict(R)
        rfit2 = SurrogateCons1.predict(R)
        rfit3 = SurrogateCons2.predict(R)
        qfit1 = SurrogateObj.predict(Q)
        qfit2 = SurrogateCons1.predict(Q)
        qfit3 = SurrogateCons2.predict(Q)
        for i in range(len(R)):
    ...
    def ISRAM(self, iter):
        m = 10
        pfit1 = SurrogateObj.predict(self.pop)
        pfit2 = SurrogateCons1.predict(self.pop)
        pfit3 = SurrogateCons2.predict(self.pop)
        tempp = []
        tempc = []
...
def find(arr, data):
    ...
    t1 = np.append(SurrogateObj.predict(arr), np.square(min(0, SurrogateCons1.predict(arr))) + np.square(min(0, SurrogateCons2.predict(arr))))
    for i in range(len(data)):
        t2 = np.column_stack((SurrogateObj.predict(data[i][np.newaxis, :]), np.square(min(0, SurrogateCons1.predict(data[i][np.newaxis, :])) + np.square(min(0, SurrogateCons2.predict(data[i][np.newaxis, :])))))).ravel()
        if dominate(t1, t2):
            temp[i] = 1
    return temp
...
class SurrogateObj():
    def predict(arr):
        ...
        return m0.predict(arr)
class SurrogateCons1():
    def predict(arr):
        '''
        :return: The first constraint value predicted by the surrogate model
        '''
        global Eva_num
        if (arr not in dataLibrary):
            Eva_num -= 1
            np.append(dataLibrary,arr)
        return m1.predict(arr)
class SurrogateCons2():
    def predict(arr):
        '''
        :return: The second constraint value predicted by the surrogate model
        '''
        global Eva_num
        if (arr not in dataLibrary):
            Eva_num -= 1
            np.append(dataLibrary,arr)
        return m2.predict(arr)
    ...
    for funname in ['ellipsoid01']:
        for dimension in [10]:
            ...
            for testnumber in range(1):
                tt0 = time.time()
                x = latin(N=11*dimension, D=dimension, lower_bound=-5.12, upper_bound=5.12)
                dataLibrary = x
                Eva_num = 10000
                y, cons = fun(x)
                cons1 = cons[:,0]
                ...
                m0.fit(x, y)
                m1 = RBFN(input_shape=dimension, hidden_shape=int(np.sqrt(len(x))), kernel='gaussian')
                m1.fit(x, cons1)
                m2 = RBFN(input_shape=dimension, hidden_shape=int(np.sqrt(len(x))), kernel='gaussian')
                m2.fit(x, cons2)
